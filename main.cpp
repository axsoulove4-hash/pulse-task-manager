...
class Window:public QMainWindow {
 ...
 QString tempText() const {QStringList lines;for(const auto &zone:QDir("/sys/class/thermal").entryList({"thermal_zone*"},QDir::Dirs)){QString base="/sys/class/thermal/"+zone+"/";auto raw=QString::fromUtf8(readFile(base+"temp")).trimmed();bool ok=false;double value=raw.toDouble(&ok);if(!ok)continue;if(value>1000)value/=1000.;if(value< -40||value>150)continue;auto type=QString::fromUtf8(readFile(base+"type")).trimmed();if(type.isEmpty())type="unknown";QString name=type;if(type.compare("x86_pkg_temp",Qt::CaseInsensitive)==0||type.compare("TCPU",Qt::CaseInsensitive)==0)name=T("CPU","CPU");else if(type.compare("acpitz",Qt::CaseInsensitive)==0)name=T("Carte mère / ACPI","Motherboard / ACPI");else if(type.startsWith("SEN",Qt::CaseInsensitive))name=T("Sonde carte mère "+type.mid(3),"Motherboard sensor "+type.mid(3));else if(type.startsWith("iwlwifi",Qt::CaseInsensitive))name=T("Carte Wi‑Fi","Wi‑Fi card");else if(type.compare("INT3400 Thermal",Qt::CaseInsensitive)==0)name=T("Gestion thermique Intel","Intel thermal management");lines<<name+" — "+QString::number(value,'f',1)+" °C  ("+type+", "+zone+")";}return lines.isEmpty()?T("Aucune sonde thermique valide disponible.","No valid thermal sensor available."):lines.join("\n");}
