...
class Window:public QMainWindow {
 ...
 QColor accent;QString theme;bool compact=false;bool english=false;QHash<int,Proc> rows;
 ...
 auto overview=new QWidget;auto ov=new QVBoxLayout(overview);ov->setContentsMargins(20,18,20,18);heading(ov,T("Performances","Performance"),T("Utilisation des ressources","Resource usage"));auto grid=new QGridLayout;int c=0;for(auto title:{T("Processeur","CPU"),T("Mémoire","Memory"),T("Réseau","Network"),T("Disques","Disks")}){auto chart=new Chart(title);chart->percent=c<2;charts<<chart;grid->addWidget(chart,c/2,c%2);++c;}ov->addLayout(grid,1);auto info=new QLabel;info->setWordWrap(true);QString cpu;for(auto line:readFile("/proc/cpuinfo").split('\n'))if(line.startsWith("model name")){cpu=QString::fromLocal8Bit(line.mid(line.indexOf(':')+1)).trimmed();break;}info->setText(cpu+"\n"+QString::number(sysconf(_SC_NPROCESSORS_ONLN))+" processeurs logiques · "+QSysInfo::prettyProductName()+" · "+QSysInfo::kernelVersion());info->setObjectName("muted");ov->addWidget(info);pages->addWidget(overview);
 ...
 void refresh(){latest=monitor.sample();double mem=latest.total?100.*(latest.total-latest.available)/latest.total:0;charts[0]->push(latest.cpu,QString::number(latest.cpu,'f',1)+" %",QString::number(sysconf(_SC_NPROCESSORS_ONLN))+" processeurs logiques");charts[1]->push(mem,QString::number(mem,'f',1)+" %",bytes(latest.total-latest.available)+" / "+bytes(latest.total));charts[2]->push(latest.rx+latest.tx,bytes(latest.rx+latest.tx)+"/s","↓ "+bytes(latest.rx)+"/s    ↑ "+bytes(latest.tx)+"/s");charts[3]->push(latest.rd+latest.wr,bytes(latest.rd+latest.wr)+"/s","Lecture "+bytes(latest.rd)+"/s · Écriture "+bytes(latest.wr)+"/s");status->setText(QString("  %1 processus  ·  CPU %2 %  ·  RAM %3  ·  Swap %4 / %5").arg(latest.processes.size()).arg(latest.cpu,0,'f',1).arg(bytes(latest.total-latest.available),bytes(latest.swap-latest.swapFree),bytes(latest.swap)));populate();}
