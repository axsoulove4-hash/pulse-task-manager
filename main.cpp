...
struct Sample {double cpu=0;quint64 total=0,available=0,swap=0,swapFree=0;double rx=0,tx=0,rd=0,wr=0;QList<Proc> processes;};
class Monitor {
 quint64 oldTotal=0,oldIdle=0,oldRx=0,oldTx=0,oldRd=0,oldWr=0;QHash<int,Proc> previous;QElapsedTimer clock; qint64 last=0;
 public: Monitor(){clock.start();} Sample sample(){Sample s; auto now=clock.elapsed();double elapsed=qMax(1LL,now-last)/1000.;last=now;
 auto stat=readFile("/proc/stat").split('\n').value(0).simplified().split(' ');quint64 total=0,idle=0;for(int i=1;i<qMin(9,int(stat.size()));++i)total+=stat[i].toULongLong();idle=stat.value(4).toULongLong()+stat.value(5).toULongLong();
 quint64 delta=total-oldTotal;if(oldTotal&&delta)s.cpu=qBound(0.,100.*(1.-double(idle-oldIdle)/delta),100.);oldTotal=total;oldIdle=idle;
...
class Window:public QMainWindow {
 ...
 QColor accent;QString theme;bool compact=false;bool english=false;QLabel *gpuInfo=nullptr,*tempInfo=nullptr;QHash<int,Proc> rows;
 ...
 QString gpuText() const {QStringList lines;for(const auto &path:QDir("/sys/class/drm").entryList({"card*"},QDir::Dirs)){QString base="/sys/class/drm/"+path+"/device/";auto vendor=QString::fromUtf8(readFile(base+"vendor")).trimmed();auto busy=QString::fromUtf8(readFile(base+"gpu_busy_percent")).trimmed();auto used=QString::fromUtf8(readFile(base+"mem_info_vram_used")).trimmed();auto total=QString::fromUtf8(readFile(base+"mem_info_vram_total")).trimmed();if(vendor.isEmpty()&&busy.isEmpty())continue;QString line=path;if(!vendor.isEmpty())line+=" · "+vendor;if(!busy.isEmpty())line+="\nActivité / Activity : "+busy+" %";if(!used.isEmpty()&&!total.isEmpty())line+="\nVRAM : "+bytes(used.toULongLong())+" / "+bytes(total.toULongLong());lines<<line;}return lines.isEmpty()?T("Aucune télémétrie GPU exposée par le pilote Linux.","No GPU telemetry exposed by the Linux driver."):lines.join("\n\n");}
 QString tempText() const {QStringList lines;for(const auto &zone:QDir("/sys/class/thermal").entryList({"thermal_zone*"},QDir::Dirs)){QString base="/sys/class/thermal/"+zone+"/";auto raw=QString::fromUtf8(readFile(base+"temp")).trimmed();bool ok=false;double value=raw.toDouble(&ok);if(!ok)continue;auto type=QString::fromUtf8(readFile(base+"type")).trimmed();lines<<type+" : "+QString::number(value/1000.,'f',1)+" °C";}return lines.isEmpty()?T("Aucune sonde thermique disponible.","No thermal sensor available."):lines.join("\n");}
 void refresh(){latest=monitor.sample();double mem=latest.total?100.*(latest.total-latest.available)/latest.total:0;charts[0]->push(latest.cpu,QString::number(latest.cpu,'f',1)+" %",QString::number(sysconf(_SC_NPROCESSORS_ONLN))+" processeurs logiques");charts[1]->push(mem,QString::number(mem,'f',1)+" %",bytes(latest.total-latest.available)+" / "+bytes(latest.total));charts[2]->push(latest.rx+latest.tx,bytes(latest.rx+latest.tx)+"/s","↓ "+bytes(latest.rx)+"/s    ↑ "+bytes(latest.tx)+"/s");charts[3]->push(latest.rd+latest.wr,bytes(latest.rd+latest.wr)+"/s","Lecture "+bytes(latest.rd)+"/s · Écriture "+bytes(latest.wr)+"/s");if(gpuInfo)gpuInfo->setText(gpuText());if(tempInfo)tempInfo->setText(tempText());status->setText(QString("  %1 processus  ·  CPU %2 %  ·  RAM %3  ·  Swap %4 / %5").arg(latest.processes.size()).arg(latest.cpu,0,'f',1).arg(bytes(latest.total-latest.available),bytes(latest.swap-latest.swapFree),bytes(latest.swap)));populate();}
